...
package body Flight.Plan is
   ...
   -- (See specification file)
   --===========================================================================
   function Get_Elevation (This : Waypoint_Record) return String is
   begin

      return Float_Image (This.Elevation, 0);
   ...
   Route_Description_Buffer : Utility.Strings.String_Buffer (6 + 33 * Waypoint_Range'Range_Length);
      ...
      if Modified then
         ...
         for Plan of Flight_Plans loop
            ...
            if Plan.Is_Loaded then
               ...
               Route_Description_Buffer.Reset;

               Route_Description_Buffer.Append (Trim (Plan.Name));

               for Waypoint of Plan.Waypoints loop

                  if Waypoint.Is_Loaded then

                     Route_Description_Buffer.Append (":");
                     Route_Description_Buffer.Append (Trim (Waypoint.Name));
                     Route_Description_Buffer.Append (">");
                     Route_Description_Buffer.Append (Image (Waypoint.Position));

                  end if;

               end loop;

               Utility.Storage.Set_Item ("PLAN" & Integer_Image (P), Route_Description_Buffer.Get_Content);
   ...
   procedure Update_Flight_Plan is
      ...
      Dist   : Float;
      Waypoint_Was_In_Range : Boolean := False;
   ...
   begin
      ...
      for W in Waypoint_Range loop
         ...
         Waypoint_Was_In_Range := Wpt.In_Range = Range_Safe;

         if Wpt.Margin = No_Altitude then

            Wpt.In_Range := Range_Unreachable;

         else

            if Wpt.Margin > Safety_Height then

               Wpt.In_Range := Range_Safe;

            elsif Wpt.Margin > 0.0 then

               Wpt.In_Range := Range_Unsafe;
         ...
         if W = 1 and then Waypoint_Was_In_Range and then not (Wpt.In_Range = Range_Safe) then

            Utility.Notifications.Add_Notification (Utility.Notifications.Notify_Range);
   ...
   -- (See specification file)
   --===========================================================================
   function Get_Radius (This : Flight_Plan_Record) return String is
   begin
   ...
   -- (See specification file)
   --===========================================================================
   function Next_Waypoint return not null access Waypoint_Record is
   begin
   ...
   -- (See specification file)
   --===========================================================================
   function Home_Waypoint return not null access Waypoint_Record is
   begin
