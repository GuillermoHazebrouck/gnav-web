...
package body Gnav is
   ...
   begin
      ...
      Utility.Strings.Override (Gnav_Info.User_Tm, Utility.Storage.Get_Item ("TM"));
      Utility.Log.Put_Message ("Tm loaded: " & Gnav_Info.User_Id);

      Flight.Set_Time_Zone (Integer_Value (Utility.Storage.Get_Item ("UTC_OFFSET"), 0));

      Gnav_Info.Altitude_Bias := Float_Value (Utility.Storage.Get_Item ("BIAS"), 50.0);

      Maps.Terrain.Set_High_Resolution (Utility.Storage.Get_Item ("RESOLUTION") = "HIGH");
   ...
   begin
      ...
      begin
         ...
         S.Space     := 0.038;
         S.Width     := 0.060;
         S.Rendering := Glex.Fonts.Font_Simple;

         Glex.Fonts.Draw
           (Gnav_Info.Core_Version,
            X         => 0.5,
            Y         => 0.3,
            Style     => S,
            Color     => Glex.Colors.Line_Gray,
            Alignment => Glex.Fonts.Alignment_CC);
   ...
   begin
      ...
      if Alt in 0.0..12_000.0 then
         Flight.Data.Altitude := Float'Max (0.0, Float (Alt) - Gnav_Info.Altitude_Bias);
   ...
   -- (See specification file)
   --===========================================================================
   function Gnav_Notification_Request return Interfaces.Integer_64 is
   begin
