...
package body Flight.Parsing is
   ...
   begin
      ...
      --------------------------------------------------------------------------
      Flight.Data.Distance := Flight.Data.Distance + Distance (Previous_Position, Flight.Data.Position);
      ...
      if abs (Cached_Time - Last_Time_Sync) > 45.0 then
         ...
         if Value'Length > 5 then
            ...
            begin
               ...
               if abs (Time_Offset - Offset) > 15.0 then
   ...
   -- Synchronizes the indicated time to the UTC time
   --===========================================================================
   procedure Synchronize_Date (Value : String) is
   begin

      -- Maximum one synchronization per 45s
      -----------------------------------------------
      if abs (Cached_Time - Last_Date_Sync) > 45.0 then

         if Value'Length > 5 then

            declare

               -- Data must be in DDMMYY format
            ...
            begin
               ...
               Gps_Time := Ada.Calendar.Time_Of (Year    => Year_Number'Value  ("20" & Value (Value'First+4..Value'First+5)),
                                                 Month   => Month_Number'Value (Value (Value'First+2..Value'First+3)),
                                                 Day     => Day_Number'Value   (Value (Value'First  ..Value'First+1)),
                                                 Seconds => Seconds (Cached_Time));

               Offset := Gps_Time - Cached_Time;

               if abs (Offset) > 0.0 then

                  Date_Offset := Offset;

               end if;

               Last_Date_Sync := Cached_Time;

               Utility.Log.Put_Message ("date synchronization done:" & Duration'Image (Date_Offset));

            exception
               when others =>
                  Date_Offset := 0.0;
                  Utility.Log.Put_Message ("date synchronization failed");
